...
function SpeakerAdapter(bus) {
  ...
  this.audio_context.pipe(new OutputSpeaker());
...
function SpeakerWorkletDAC(bus, audio_context, mixer) {
  ...
  if (SPEAKER_DEBUG) {
    worklet_code = "var SPEAKER_DEBUG = true;\n" + worklet_code;
  }
  ...
  if (SPEAKER_DEBUG) {
    this.SPEAKER_DEBUGger = new SpeakerDACSPEAKER_DEBUGger(this.audio_context, this.node_output);
  }
}
...
SpeakerWorkletDAC.prototype.queue = function(data) {
  ...
  if (SPEAKER_DEBUG) {
    this.SPEAKER_DEBUGger.push_queued_data(data);
  }
 ...
 * @constructor
 */
function SpeakerDACSPEAKER_DEBUGger(audio_context, source_node) {
  /** @const */
  this.audio_context = audio_context;
...
/** @suppress {deprecated} */
SpeakerDACSPEAKER_DEBUGger.prototype.start = function(duration_ms) {
  this.is_active = true;
  this.queued = [
    [],
    []
...
SpeakerDACSPEAKER_DEBUGger.prototype.stop = function() {
  this.is_active = false;
...
SpeakerDACSPEAKER_DEBUGger.prototype.push_queued_data = function(data) {
  if (this.is_active) {
...
SpeakerDACSPEAKER_DEBUGger.prototype.download_txt = function(history_id, channel) {
...
SpeakerDACSPEAKER_DEBUGger.prototype.download_csv = function(history_id) {
